...
- `compute_spatial_linkage.py`: performs the linkage classification on polygons via the analysis of shared perimeter ratios.
...
- Produces a single combined assessment layer (`ADD_polys_with_RGI-GCv7_IRRv1_combined.gpkg`) which will be used for linkage computation, each polygon can be track back to the RGI-ids and IRR-ids.
